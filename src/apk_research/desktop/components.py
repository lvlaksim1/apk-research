...
def default_component_root() -> Path:
    ...
    if local_app_data:
        return Path(local_app_data) / "apk-research" / "components"
    return Path.home() / ".apk-research" / "components"
...
def select_archive_from_repository_xml(
    ...
    *,
    ...
    from apk_research.desktop.repository_policy import (
...
class ComponentManager:
    ...
    def remove_all(self) -> None:
        """Remove every apk-research managed Android component.
    ...
    def _download_archive(
        ...
        request = urllib.request.Request(
            archive.url,
            headers={"User-Agent": "apk-research/0.2"},
        )
        try:
